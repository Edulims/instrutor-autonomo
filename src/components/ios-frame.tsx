// ios-frame.tsx — Simplified iOS 26 (Liquid Glass) device frame. No assets, no deps.
...
function IOSGlassPill({ children, dark = false, style = {} }: { children: ReactNode; dark?: boolean; style?: CSSProperties }) {
  return (
    <div
      style={{
        height: 44, minWidth: 44, borderRadius: 9999, position: 'relative', overflow: 'hidden',
...
// ── Navigation bar ──────────────────────────────────────────
function IOSNavBar({ title = 'Title', dark = false, trailingIcon = true }: { title?: string; dark?: boolean; trailingIcon?: boolean }) {
...
function IOSKeyboard({ dark = false }: { dark?: boolean }) {
    ...
    return (
      <div
        ...
        style={{
          ...
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontFamily: '-apple-system, "SF Compact", system-ui', fontSize: fs, fontWeight: 458, color: ret ? '#fff' : glyph,
        }}
      >
        {content}
      </div>
